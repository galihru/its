...
object TrafficSignalController {
  def fromEnv(vehicleCount: () => Int): TrafficSignalController = {
    ...
    new TrafficSignalController(
      backend = GpioBackend.auto(enabled, pins),
      pins = pins,
...
final class TrafficSignalController(
  ...
  private def runPhase(color: String, durationSec: Int): Unit = {
    if (!running) return
...
object GpioBackend {
  def auto(enabled: Boolean, pins: TrafficPins): GpioBackend = {
    if (!enabled) return new NoopGpioBackend("disabled", pins)
    if (!isLinux) return new NoopGpioBackend("not-linux", pins)

    commandPath("pinctrl")
      .map(path => new PinctrlGpioBackend(path, pins): GpioBackend)
      .orElse(commandPath("raspi-gpio").map(path => new RaspiGpioBackend(path, pins): GpioBackend))
      .getOrElse {
        if (Files.exists(Paths.get("/sys/class/gpio/export"))) new SysfsGpioBackend(pins)
...
abstract class CommandGpioBackend(command: String, pins: TrafficPins) extends GpioBackend {
  ...
  private def writePin(pin: Int, high: Boolean): Unit = {
    val process = new ProcessBuilder((Seq(command) ++ pinArgs(pin, high)): _*)
      .redirectErrorStream(true)
      .start()
    val exitCode = process.waitFor()
    if (exitCode != 0) {
      val output = new String(process.getInputStream.readAllBytes(), StandardCharsets.UTF_8).trim
      println(s"[${java.time.LocalDateTime.now()}] GPIO command failed ($name pin=$pin high=$high): $output")
    }
  }
}

final class PinctrlGpioBackend(command: String, pins: TrafficPins)
  extends CommandGpioBackend(command, pins) {
...
final class RaspiGpioBackend(command: String, pins: TrafficPins)
  extends CommandGpioBackend(command, pins) {
...
final class SysfsGpioBackend(pins: TrafficPins) extends GpioBackend {
  ...
  private def writePin(pin: Int, high: Boolean): Unit =
    write(Paths.get(s"/sys/class/gpio/gpio$pin/value"), if (high) "1" else "0")
  ...
  private def write(path: Path, value: String): Unit =
    try Files.writeString(path, value, StandardCharsets.UTF_8, StandardOpenOption.WRITE)
      ...
      case NonFatal(ex) =>
        println(s"[${java.time.LocalDateTime.now()}] GPIO sysfs write failed: $path: ${ex.getMessage}")
    }
}
...
final class NoopGpioBackend(reason: String, pins: TrafficPins) extends GpioBackend {
  ...
  override def initialize(): Unit =
    println(s"[${java.time.LocalDateTime.now()}] GPIO disabled: $reason (red=${pins.red}, yellow=${pins.yellow}, green=${pins.green})")
