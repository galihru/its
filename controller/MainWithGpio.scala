...
object ItsController {
  ...
  @volatile private var trafficDurationSec: Int = 4

  def main(args: Array[String]): Unit = {
    // Initialize GPIOs and start traffic light thread
    initGpios()
    startTrafficThread()
  ...
  private def startTrafficThread(): Unit = {
    val t = new Thread(() => {
      val rnd = new scala.util.Random()
      while (true) {
        try {
          val r = 4 + rnd.nextInt(5)
          setTrafficColor("red")
          trafficStartedAt = System.currentTimeMillis()
          trafficDurationSec = r
          writeSnapshot()
          Thread.sleep(r * 1000L)

          val y = 2 + rnd.nextInt(3)
          setTrafficColor("yellow")
          trafficStartedAt = System.currentTimeMillis()
          trafficDurationSec = y
          writeSnapshot()
          Thread.sleep(y * 1000L)

          val g = 3 + rnd.nextInt(6)
          setTrafficColor("green")
          trafficStartedAt = System.currentTimeMillis()
          trafficDurationSec = g
          writeSnapshot()
          Thread.sleep(g * 1000L)
        } catch {
          case _: InterruptedException => return
          case ex: Throwable => println(s"[${java.time.LocalDateTime.now()}] traffic thread error: ${ex.getMessage}")
        }
      }
    })
    t.setDaemon(true)
  ...
  private def setTrafficColor(color: String): Unit = {
    currentTrafficColor = color
    color match {
      case "red"    => writeGpioValue(redPin, 1); writeGpioValue(yellowPin, 0); writeGpioValue(greenPin, 0)
      case "yellow" => writeGpioValue(redPin, 0); writeGpioValue(yellowPin, 1); writeGpioValue(greenPin, 0)
      case "green"  => writeGpioValue(redPin, 0); writeGpioValue(yellowPin, 0); writeGpioValue(greenPin, 1)
      case _         => writeGpioValue(redPin, 0); writeGpioValue(yellowPin, 0); writeGpioValue(greenPin, 0)
    }
  }

  // ─── GPIO Control (sysfs with shell fallback) ──────────────────────────
  private def gpioPath(pin: Int, file: String): Path = Paths.get(s"/sys/class/gpio/gpio$pin/$file")

  private def exportPin(pin: Int): Unit = try {
    val p = Paths.get(s"/sys/class/gpio/gpio$pin")
    if (!Files.exists(p)) {
      try {
        Files.writeString(Paths.get("/sys/class/gpio/export"), pin.toString, StandardCharsets.UTF_8)
        Thread.sleep(100)
        if (Files.exists(p)) {
          println(s"[${java.time.LocalDateTime.now()}] GPIO $pin exported successfully")
        } else {
          println(s"[${java.time.LocalDateTime.now()}] Warning: GPIO $pin export written but folder not created")
        }
      } catch { case ex: Throwable =>
        println(s"[${java.time.LocalDateTime.now()}] GPIO $pin export failed: ${ex.getMessage}")
      }
    } else {
      println(s"[${java.time.LocalDateTime.now()}] GPIO $pin already exported")
    }
  } catch { case ex: Throwable =>
    println(s"[${java.time.LocalDateTime.now()}] GPIO $pin export error: ${ex.getMessage}")
  }

  private def setGpioDirection(pin: Int, dir: String): Unit = try {
    val p = gpioPath(pin, "direction")
    if (Files.exists(p)) {
      Files.writeString(p, dir, StandardCharsets.UTF_8)
      println(s"[${java.time.LocalDateTime.now()}] GPIO $pin direction set to $dir")
    } else {
      println(s"[${java.time.LocalDateTime.now()}] Warning: GPIO $pin direction file not found at $p")
    }
  } catch { case ex: Throwable =>
    println(s"[${java.time.LocalDateTime.now()}] GPIO $pin direction error: ${ex.getMessage}")
  }

  private def writeGpioValue(pin: Int, value: Int): Unit = try {
    val p = gpioPath(pin, "value")
    if (Files.exists(p)) {
      Files.writeString(p, value.toString, StandardCharsets.UTF_8)
    } else {
      println(s"[${java.time.LocalDateTime.now()}] Warning: GPIO $pin value file not found, trying shell fallback")
      try {
        val cmd = Array("/bin/sh", "-c", s"echo $value > /sys/class/gpio/gpio$pin/value")
        val proc = new ProcessBuilder(cmd*).redirectErrorStream(true).start()
        proc.waitFor()
        println(s"[${java.time.LocalDateTime.now()}] GPIO $pin value written via shell (value=$value)")
      } catch { case ex: Throwable =>
        println(s"[${java.time.LocalDateTime.now()}] GPIO $pin shell fallback failed: ${ex.getMessage}")
      }
    }
  } catch { case ex: Throwable =>
    println(s"[${java.time.LocalDateTime.now()}] GPIO $pin write error: ${ex.getMessage}")
  }

  private def initGpios(): Unit = {
    try {
      println(s"[${java.time.LocalDateTime.now()}] Initializing GPIO pins: red=$redPin yellow=$yellowPin green=$greenPin")
      Seq(redPin, yellowPin, greenPin).foreach { pin =>
        exportPin(pin)
        setGpioDirection(pin, "out")
        writeGpioValue(pin, 0)
      }
      println(s"[${java.time.LocalDateTime.now()}] GPIO initialization complete")
    } catch { case ex: Throwable =>
      println(s"[${java.time.LocalDateTime.now()}] GPIO init error: ${ex.getMessage}")
      ex.printStackTrace()
    }
  }
  ...
  private def buildJsonPair(): (String, String) = {
    ...
    val deviceJson =
      s"""{
         ...
         |  "trafficColor": "${escapeJson(currentTrafficColor)}",
         |  "trafficStartedAt": ${trafficStartedAt},
         |  "trafficDurationSec": ${trafficDurationSec},
         |  "position": {
         |    "lat": ${location.lat},
         |    "lng": ${location.lng}
         |  }
