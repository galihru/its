...
function selectedDevice(): DeviceRecord {
  return state.devices.find((device) => device.id === state.selectedId) || state.devices[0] || FALLBACK.devices[0]!;
}
...
function render(): void {
  ...
  const averageCongestion = Math.round(state.devices.reduce((sum, device) => sum + device.congestion, 0) / Math.max(1, state.devices.length));
  ...
  try {
    ...
    if (Array.isArray(snapshot.devices) && snapshot.devices.length) {
      state.devices = snapshot.devices.map((device, index) => {
        const fallback = FALLBACK.devices[index % FALLBACK.devices.length] || FALLBACK.devices[0]!;
        return {
          id: String(device.id || fallback.id),
          label: String(device.label || fallback.label),
          district: String(device.district || fallback.district),
          ip: String(device.ip || ""),
          status: (device.status as DeviceStatus) || fallback.status,
          ...
          position: {
            x: Number(device.position?.x ?? fallback.position.x),
            y: Number(device.position?.y ?? fallback.position.y),
          },
        };
      });
    }
