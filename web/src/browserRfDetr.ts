...
const RF_DETR_MODEL_ID = "onnx-community/rfdetr_medium-ONNX";
...
export function drawRfDetrDetections(
  ...
  ctx.clip();
  if (options.hud !== false) drawHudGuide(ctx, canvas.width, canvas.height, ratio, tracks.length);
  const focusDetection = options.scannerFocus || tracks[0] || null;
  const scannerFocus = focusDetection
    ? {
      x: (focusDetection.x + focusDetection.width / 2) * scale + offsetX,
      y: (focusDetection.y + focusDetection.height / 2) * scale + offsetY,
      width: focusDetection.width * scale,
      height: focusDetection.height * scale,
    }
    : null;
  if (!tracks.length) {
    if (options.scanActive !== false) drawCinematicScanner(ctx, canvas.width, canvas.height, ratio, now, 0.95, scannerFocus);
  }
  else {
    if (options.scanActive) drawCinematicScanner(ctx, canvas.width, canvas.height, ratio, now, 0.62, scannerFocus);
    drawScannerLocks(ctx, tracks, scale, offsetX, offsetY, ratio, now);
  }
  tracks.slice(0, RENDER_TRACK_LIMIT).forEach((det) => {
    if (!isRenderableDetection(det, frameWidth, frameHeight)) return;
    const age = Math.max(0, now - det.createdAt);
    const grow = easeOutCubic(clamp(age / 720, 0, 1));
    ...
    const confirmed = grow >= 0.72 && (det.seen >= 2 || confidence >= 0.34);
    ...
    if (grow < 1) {
      ...
      ctx.globalAlpha = clamp(det.alpha, 0, 1) * 0.82;
    ...
    drawDetectionTicks(ctx, left, top, boxWidth, boxHeight, ratio, stroke);
    if (!confirmed) {
      ctx.globalAlpha = 1;
      return;
    }
...
export async function runBrowserRfDetr(source: ImageSource): Promise<BrowserRfDetrResult> {
  ...
  try {
    return await runRfDetrFrame(frame, startedAt);
  } catch (rfDetrError) {
...
export async function publishBrowserRfDetrResult(
  ...
  const vehicleOnly = {
    ...
    objectCount: result.vehicleBreakdown.total,
...
async function runRfDetrFrame(frame: CapturedFrame, startedAt: number): Promise<BrowserRfDetrResult> {
  ...
  if (shouldRunDetailCrops(frame, detections)) {
    const crops = detailCropsForFrame(frame).slice(0, RF_DETR_DETAIL_CROP_LIMIT);
  ...
  const note = detections.length
    ? `${activeObjectDetectionModelId === RF_DETR_MODEL_ID ? "RF-DETR" : "DETR fallback"} browser mendeteksi objek`
    : rfDetrLoadingNote || `${activeObjectDetectionModelId === RF_DETR_MODEL_ID ? "RF-DETR" : "DETR fallback"} browser aktif, masih memindai objek`;
...
async function loadRfDetrPipeline(): Promise<RfDetrPipeline> {
  if (rfDetrPipelinePromise) return rfDetrPipelinePromise;
  rfDetrPipelinePromise = (async () => {
    ...
    try {
      const detector = await createRfDetrManualDetector(mod, quantizedOptions);
      activeObjectDetectionModelId = RF_DETR_MODEL_ID;
      return detector;
    } catch (error) {
...
async function createRfDetrManualDetector(
  ...
  const processor = await mod.AutoProcessor.from_pretrained(RF_DETR_MODEL_ID, options);
  const model = await mod.RFDetrForObjectDetection.from_pretrained(RF_DETR_MODEL_ID, options);
  const detector = (async (image: HTMLCanvasElement, detectorOptions?: { threshold?: number; percentage?: boolean }) => {
...
async function inferRfDetrFrame(
  ...
  return output.flatMap((item) => {
    ...
    if (!label || score < confidenceForLabel(label)) return [];
    ...
    const detection = normalizeDetection({ label, confidence: score, x: xmin, y: ymin, width, height }, frame.width, frame.height);
...
function detailCropsForFrame(frame: CapturedFrame): DetectionCrop[] {
  ...
  return crops.flatMap((crop) => cropImageData(frame.canvas, crop.x, crop.y, crop.width, crop.height));
}
...
function detectionMatchesClassGeometry(det: BrowserRfDetrDetection, frameWidth: number, frameHeight: number): boolean {
  ...
  if (det.confidence < adaptiveConfidenceForDetection(det, frameWidth, frameHeight)) return false;
  if (label === "person") {
  ...
  const preferredBoost = label === "person"
    ...
    : VEHICLE_LABELS.has(label)
      ...
      : label === "traffic light" || label === "stop sign"
        ? 0.08
        : 0;
  const penalty = label === "parking meter" || label === "spoon" || label === "fork" || label === "knife" ? 0.28 : 0;
...
function adaptiveConfidenceForDetection(det: BrowserRfDetrDetection, frameWidth: number, frameHeight: number): number {
  ...
  return clamp(base - smallObjectBoost, 0.055, 0.52);
}
